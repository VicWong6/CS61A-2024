...
def pick(paragraphs, select, k):
    ...
    for idx, item in enumerate(paragraphs):
        # print(f'DEBUG: k:{k}, idx:{idx}, item:{item}')
        # print(f'DEBUG: select(item) is {select(item)}')
        if select(item):
            find += item
    if k >= len(find) :
        return ''
    ...
    # implement with LIst Comprehensions 
    find = [ x for x in paragraphs if select(x) ]
    if k >= len(find):
        return ''
    else:
        return find[k]
...
def about(subject):
    ...
    def is_hint(sentence):
        # sentence is a string, transfer string to list, using remove_punctuation, lower, and split,
        # targets is a list , e.g. ['its', 'a', 'lovely', 'day']
        targets = split(lower(remove_punctuation(sentence)))
        for sub in subject: # subject also is a list. e.g.  ['dog', 'dogs', 'pup']
            # The preceding assert has already ensured that subject is in lowercase.
            if sub in targets:
                return True
        return False
...
def accuracy(typed, source):
    ...
    if is_emp_list(source_words):
        ...
        if is_emp_list(typed_words):
            return 100.0
        else:
            return 0.0
    elif is_emp_list(typed_words):
        return 0.0
    else:
        hint, total = 0, len(typed_words)
        min_len = min(len(typed_words), len(source_words))
        for idx in range(min_len):
            if typed_words[idx] == source_words[idx]:
                hint += 1
        return hint / total * 100
