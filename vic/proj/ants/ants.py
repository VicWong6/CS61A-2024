...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            cur_ant = place.ant
            if self.is_container and self.can_contain(cur_ant):
                self.store_ant(cur_ant)
                place.ant = self # ignore the warning in vscode  : unreachable code
            elif cur_ant.is_container and cur_ant.can_contain(self):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        """
        cur_place = self.place # current place
        while True:
            if cur_place.is_hive: # hive is the end of places,return None
                return None
            if cur_place.bees: # find bees in this place
                return random_bee(cur_place.bees)
            else: # find next place
                cur_place = cur_place.entrance
        """

        # solution for Probelem 4
        cur_place = self.place # current place
        dis = 0 # distance from ant place to nearest bee
        while True:
            if cur_place.is_hive: # hive is the end of places,return None
                return None
            elif dis < self.lower_bound:
                dis += 1
                cur_place = cur_place.entrance
            elif dis > self.upper_bound:
                return None # can't attack too far
            else: # lower_bound <= dis <= upper_bound
                if cur_place.bees: # find bees in this place
                    return random_bee(cur_place.bees)
                else:
                    dis += 1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # 1. 这一步是关键：先处理周围蜜蜂的伤害计算
        # 注意：要在蚂蚁可能死亡之前（或同时）处理逻辑
...
class HungryAnt(Ant):
    ...
    food_cost = 4
    implemented = True # Change to True to view in the GUI
    chew_cooldown = 3 # default 3

    def __init__(self, health=1):
        self.cooldown = 0 # don't eat bee, so init 0
        super().__init__(health)
    ...
    def action(self, gamestate):
        ...
        if self.cooldown > 0:
            ...
            bee_to_kill = random_bee(self.place.bees)
            bee_to_kill.reduce_health(bee_to_kill.health)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if self.ant_contained: # 已存储了一个ant，不能再存了
            return False
        elif not isinstance(other, Ant) or other.is_container: # 不是蚂蚁类 或者 另一个也是容器蚂蚁
            return False
        else:
            return True
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        place_bees = list(self.place.bees) # must create new list, to avoid changing the list while traversing
        # print(f"DEBUG: self damage:{self.damage}, bees:{self.place.bees}")
        if place_bees:
            for bee in place_bees:
                # print(f"DEBUG: attack one :{bee}")
...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self, health=1):
        super().__init__(health)
        self.enhanced_ants = [] # to store all enhanced ants behind queen
    ...
    def action(self, gamestate):
        ...
        while temp_place:
            cur_ant = temp_place.ant
            self.double_nested(cur_ant)
            temp_place = temp_place.exit

        super().action(gamestate)
        # END Problem 12

    # 容器和容器里面的蚂蚁伤害也应该加倍。递归遍历，避免以后能多层嵌套
    def double_nested(self, ant):
        if ant == None:
            return None
        if ant not in self.enhanced_ants:
            ant.double()
            self.enhanced_ants.append(ant)
        if ant.is_container:
            self.double_nested(ant.ant_contained)
