...
def buy(fruits_to_buy, prices, total_amount):
    ...
    def add(fruits, amount, cart):
        # print(f"DEBUG: ---add before---- fruits:{fruits}, amount:{amount}, cart:{cart}")
        if fruits == [] and amount == 0:
            ...
            for k in range(1, amount // price + 1):
                # print(f"DEBUG: ---IN FOR K--- k:{k}, fruits[1:]:{fruits[1:]}, remain:{amount - k * price}")
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    lat_dif = abs(get_lat(city_a) - get_lat(city_b))
    lon_dif = abs(get_lon(city_a) - get_lon(city_b))
...
def closer_city(lat, lon, city_a, city_b):
    ...
    dis_a = distance(temp_city, city_a)
    dis_b = distance(temp_city, city_b)
